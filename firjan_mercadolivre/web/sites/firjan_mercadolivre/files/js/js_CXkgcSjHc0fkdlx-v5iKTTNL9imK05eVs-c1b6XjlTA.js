      ...
      $(window).on('load', function () {
        ...
        setTimeout(() => {
          ...
          $(document).on('submit','form.cpf',function(e) {
            ...
            $.when( $.ajax( '/jsonapi/taxonomy_term/whitelist?fields[whitelist--name]=name&sort=name&page[limit]=50' ) ).done(function( response ) {
              if(response.data) console.log(response.data)
              if(response.links.next) {
                getTaxonomies(response.links.next.href).then(response => {
            ...
            //   let user = response.data.filter(taxonomy => taxonomy.attributes.name == val)[0];

            //   if(!user && response.links.next) {
            //     getTaxonomies(response.links.next.href).then(response => {
            //       console.log(response)
            //     }).catch(error => alert(error));
            //   } else {
